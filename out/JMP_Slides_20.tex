...
\begin{frame}[plain]
  ...
  {\footnotesize\color{zinc600}
    $^*$University of Arkansas\hspace{2mm}
    $^\dagger$Florida State University
  } 
  
  \vspace{20mm}
  {\footnotesize \url{https://kylebutts.com/files/JMP.pdf}}
\end{frame}
...
\begin{frame}{Motivation}
  ...
  \bigskip\pause
  Standard difference-in-differences assumption of parallel counterfactual trends is \emph{implausible}
\end{frame}
...
\begin{frame}{Modeling differential trends}
  ...
  \pause
  \bigskip
  Allows treated units to be on trends \emph{based on macroeconomic shocks}
\end{frame}
...
\begin{frame}{This paper}
  We propose a \textbf{class of imputation-style treatment effect estimators} under a \textbf{factor model} that is fixed-$T$ consistent under treatment-effect heterogeneity

  \pause
  \bigskip
  \begin{itemize}
    \item Our `imputation' style estimator explicitly estimates the untreated potential outcome, $\cranberry{y_{it}(0)}$, in the post-treatment periods 
    \begin{itemize}
...
\begin{frame}{This paper}{}
  ...
  \bottomleft{
    \hyperlink{slide:appendix-qld_details}{\beamergotobutton{Quasi-long Differencing Details}} \
...
%   \begin{equation*}
%     \cranberry{y_{it}(0)} = \mu_i + \lambda_t + X_i \beta_t + u_{it}
%   \end{equation*}% 
%   \begin{itemize}
...
%     
%     \item Noisy measures of true `exposure' only partially control for the problem \smcitep{kejriwal2024efficacy}
%   \end{itemize}
% 
%   \bottomleft{\hyperlink{slide:noisy_xi_simulations}{\beamergotobutton{Simulation Evidence}}}
% \end{frame}

\begin{frame}{Current approaches}{Synthetic-control style estimators}
  ...
  \begin{enumerate}
    ...
    \item Imputation Estimators \smcitep{Gobillon_Magnac_2016, xu2017generalized}
  \end{enumerate}
...
\begin{frame}{Importance of fixed-$T$ estimatiors}
...
\begin{frame}{Impact of new Walmart entry on $\log$ retail employment}{TWFE Imputation estimates}
  \vspace{-5mm}
  \begin{figure}
...
\begin{frame}{Impact of new Walmart entry on $\log$ retail employment}{Including $w_i \beta_t$ in TWFE imputation}
...
\begin{frame}{Impact of new Walmart entry on $\log$ retail employment}{Quasi-long differencing estimates using $w_i$ as instruments}
...
\begin{frame}{Can use many different factor-model estimators}
...
\begin{frame}{Imputation estimator allows synthetic control style plots}
...
\begin{frame}{Model}\label{slide:factor_model}
  ...
  \bottomleft{%
    \hyperlink{slide:appendix-selecting_p}{\beamergotobutton{Estimating $p$}} \ 
    \hyperlink{slide:appendix-shift_share}{\beamergotobutton{Differences with Shift-Share IV}}
  }
\end{frame}

% TODO: We place no restrictions on the distribution of treatment-timing
...
\begin{frame}{Treatment Effects}
  ...
  \begin{itemize}
    \item We assume a large number of treated and control units $N_1 = \sum_i D_i$ and $N_0 = \sum_i D_i$ are non-vanishing as $N$ grows
  \end{itemize}
  ...
  $$
    \text{ATT}_t \equiv \expec{y_{it}(1) - \raspberry{y_{it}(0)}}{D_i = 1},
  $$
  where $\raspberry{y_{it}(0)}$ is the (unobserved) untreated potential outcome.
\end{frame}
...
\begin{frame}{Assumptions}{Factor model}
  ...
  \begin{block}{}
    ...
    $$
      \cranberry{y_{it}(0)} = \bm f_t' \bm{\gamma}_i + u_{it}.
    $$
...
\begin{frame}{Assumptions}{Additional assumptions}
  ...
  \begin{block}{}
    ...
    $$    
      \tau_{it} = y_{it}(1) - \cranberry{y_{it}(0)}
    $$
  \end{block}
  ...
  \begin{block}{}
    ...
    Whenever $d_{it} = 0$, the observed $y_{it} = \cranberry{y_{it}(0)}$.
  \end{block}
\end{frame}
...
\begin{frame}{$\text{ATT}_t$ Identification}
  ...
  \begin{align*}
    ...
    &= \expec{y_{it}(1) - \cranberry{\left( \bm{f}_t' \bm{\gamma}_i + u_{it} \right)} }{D_i = 1} \\
    &= \expec{y_{it}(1)}{D_i = 1} - \cranberry{ \bm{f}_t' \expec{\bm{\gamma}_i}{D_i = 1} }
  \end{align*}  
...
\begin{frame}{$\text{ATT}_t$ Identification}
  ...
  \begin{align*}
    ...
    &\quad = \expec{y_{it} - \cranberry{y_{it}(0)}}{D_i = 1} = ATT_t,
  \end{align*}
...
\begin{frame}{Estimation of $\bm{F}$}
  ...
  \bigskip
  Consistency requires $\sqrt{n}$-consistent estimation of the column-span of the factors $\bm{F}$
...
%   In the paper, we derive asymptotic results using an instrumental-variables based identification strategy as proposed in \smcitet{Ahn_Lee_Schmidt_2013}.
%   \begin{itemize}
...
%   In the case of our Walmart application, we use the following variables as instruments:
%   \begin{itemize}
...
%   \begin{itemize}
%     \item Use 1980 baseline values helps us avoid picking up on concurrent county-shocks
%   \end{itemize}
% \end{frame}
% 
% 
% \begin{frame}{Removing TWFE additive effects}\label{slide:remove_additive_fe}
...
%     
%     \item Under QLD strategy, testing $p = 0$ is a test for sufficiency of the TWFE model
%   \end{itemize}
% 
% 
%   \bottomleft{%
...
\begin{frame}{Selection of $p$}\label{slide:appendix-selecting_p}
  ...
  \begin{itemize}
    ...
    \smallskip
    \item For principal-components, \citet{xu2017generalized}, discuss a similar selection procedure
...
\begin{frame}
  \begin{figure}
    ...
    \begin{adjustbox}{width=\textwidth}
      \includegraphics{figures/CCE_paper/trade-cce_est_interpolated.pdf}
    \end{adjustbox}
  \end{figure}
\end{frame}
...
\begin{frame}
  \begin{figure}
    ...
    \begin{adjustbox}{width=\textwidth}
      \includegraphics{figures/CCE_paper/trade-cce_mediated_est_interpolated.pdf}
    \end{adjustbox}
  \end{figure}
\end{frame}
