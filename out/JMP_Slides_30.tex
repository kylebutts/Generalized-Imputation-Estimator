...
\begin{frame}{This paper}{}
  ...
  \begin{itemize}
    ...
    \item Technically, only need to estimate the factors up to a invertible linear transformation
  \end{itemize}
...
\begin{frame}{Treatment Effects}
  ...
  \begin{itemize}
    ...
    \item We need the number of periods without any treated units, $T_0$, to be $\geq p$ for identification
  \end{itemize}
...
\begin{frame}{$\text{ATT}_t$ Identification}
  ...
  \begin{itemize}
    \item We only need to estimate $\expec{\bm{\gamma}_i}{D_i = 1}$ which is possible in small-$T$ settings, given $N_1 \to \infty$
  \end{itemize}
\end{frame}
