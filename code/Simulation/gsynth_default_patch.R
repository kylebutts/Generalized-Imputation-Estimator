...
patched_gsynth <- function(formula = NULL, data, # a data frame (long-form)
                   Y, # outcome
                   D, # treatment
...
patched_gsynth.formula <- function(formula = NULL,data, # a data frame (long-form)
                           Y, # outcome
                           D, # treatment
                           X = NULL, # time-varying covariates
                           na.rm = FALSE, # remove missing values
...
patched_gsynth.default <- function(formula = NULL,data, # a data frame (long-form)
                           Y, # outcome
                           D, # treatment
                           X = NULL, # time-varying covariates
                           na.rm = FALSE, # remove missing values
    ...
    if (se == FALSE) {
        if (MC == FALSE) {
            if (EM == FALSE) { # the algorithm suggested in the paper
                out <- gsynth:::synth.core(Y = Y, X = X, D = D, I = I, W = W,
                ...
                if (CV == FALSE) {
                    out <- gsynth:::synth.em(Y = Y, X = X, D = D, I = I, W = W,
                                    r = r, force = force,
                                    tol = tol, AR1 = AR1, norm.para = norm.para)


                } else { # cross-validation
                    out <- gsynth:::synth.em.cv(Y = Y,X = X, D = D, I = I, W = W,
                                       r = r, r.end = r.end,
            ...
            out <- gsynth:::synth.mc(Y = Y, X = X, D = D, I = I, W = W, lambda = lambda,
        ...
        out <- gsynth:::synth.boot(Y = Y, X = X, D = D, cl = cl, I=I, W = W, EM = EM,
