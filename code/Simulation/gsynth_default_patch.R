...
patched_gsynth <- function(
  ...
  UseMethod("patched_gsynth")
}

## formula method

patched_gsynth.formula <- function(
  formula = NULL,
  data, # a data frame (long-form)
  Y, # outcome
  D, # treatment
...
patched_gsynth.default <- function(
  formula = NULL,
  data, # a data frame (long-form)
  Y, # outcome
  D, # treatment
  ...
  if (length(unique_label) != dim(data)[1]) {
    stop(
      "Some records may be duplicated or wrongly marked in the data set. Check the index."
    )
  }
  ...
  if (!criterion %in% c("mspe", "pc")) {
    stop(
      "\"criterion\" option misspecified; choose from c(\"mspe\", \"pc\").\n"
    )
  }
  ...
  if (!inference %in% c("parametric", "nonparametric", "jackknife")) {
    stop(
      "\"inference\" option misspecified; choose from c(\"parametric\", \"nonparametric\", \"jackknife\")."
    )
  }
  ...
  if (
    inference == "nonparametric" && n_treated < 40 && se && estimator != "mc"
  ) {
    warning(
      "Nonparametric bootstrap may be inappropriate when there are too few 40 treated units; consider using parametric bootstrap or jackknife."
    )
  }
  ...
  if (alpha <= 0 || alpha >= 1) {
    stop(
      "\"alpha\" should be in the range of 0 and 1. Try, for example, alpha = 0.05."
    )
  }
  ...
  if (se == FALSE) {
    if (MC == FALSE) {
      if (EM == FALSE) {
        # the algorithm suggested in the paper
  ...
  colnames(out$Y.ct) <- colnames(out$Y.tr) <- colnames(out$I.tr) <- colnames(
    out$D.tr
  ) <- colnames(out$post) <- colnames(out$pre) <- iname[which(out$tr == 1)]
  rownames(out$Y.ct) <- rownames(out$Y.tr) <- rownames(out$I.tr) <- rownames(
    out$D.tr
  ) <- rownames(out$Y.co) <- rownames(out$post) <- rownames(
    out$pre
  ) <- rownames(out$Y.bar) <- rownames(Y) <- rownames(obs.missing) <- tname
  ...
  if (!is.null(out$factor)) {
    ...
    colnames(out$lambda.tr) <- colnames(out$lambda.co) <- colnames(
      out$factor
    ) <- sapply(1:dim(out$factor)[2], function(i) {
      paste("r", i, sep = "")
    })
  }
