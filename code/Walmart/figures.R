...
# %%
#| label: "Load QLD"
qld_retail = fread(here("estimates/qld_p_2_est_retail_B_5000.csv"))
...
qld_retail$lower_normal = 
  qld_retail$estimate - 1.96 * qld_retail$std_error
qld_retail$upper_normal = 
  qld_retail$estimate + 1.96 * qld_retail$std_error

qld_wholesale = fread(here("estimates/qld_p_1_est_wholesale_B_5000.csv"))
qld_wholesale = as.matrix(qld_wholesale)
qld_wholesale = qld_wholesale[apply(qld_wholesale, 1, \(x) !any(is.nan(x))), ]
...
qld_wholesale$lower_normal = 
  qld_wholesale$estimate - 1.96 * qld_wholesale$std_error
qld_wholesale$upper_normal = 
  qld_wholesale$estimate + 1.96 * qld_wholesale$std_error
...
did2s_covs_retail = fread(here("estimates/did2s_est_covs_retail.csv"))
did2s_covs_wholesale = fread(here("estimates/did2s_est_covs_wholesale.csv"))
...
# Load Naive SEs
qld_retail_naive_se = fread(here("estimates/qld_p_2_est_retail_naive_se.csv"))
qld_wholesale_naive_se = fread(here("estimates/qld_p_1_est_wholesale_naive_se.csv"))
...
synth_retail = read_csv(here("estimates/est_y0_outcome_retail_qld_p_2.csv"))
synth_retail = synth_retail |> 
  filter(g != Inf) |> 
  summarize(
...
synth_wholesale = fread(here("estimates/est_y0_outcome_wholesale_qld_p_1.csv"))
synth_wholesale = synth_wholesale |> 
  filter(g != Inf) |> 
  summarize(
    ytilde_mean = mean(ytilde, na.rm = TRUE), 
    ytilde_0hat_mean = mean(y0hat, na.rm = TRUE), 
...
(plot_did2s_retail <- ggplot() + 
  ...
  # 1.96 * standard error
  geom_errorbar(
    aes(x = rel_year, ymin = estimate - 1.96*std_error, ymax = estimate + 1.96*std_error, color = pre),
    data = did2s_retail,
    width = 0.6, linewidth = 2
  ) +
  labs(
...
kfbmisc::tikzsave(
  ...
  plot_did2s_retail, width = 10, height = 5
)
kfbmisc::tikzsave(
  here("figures/did2s_wholesale.pdf"),
  plot_did2s_wholesale, width = 10, height = 5
)

## QLD 
kfbmisc::tikzsave(
  here("figures/qld_retail.pdf"),
  plot_qld_retail, width = 10, height = 5
)
kfbmisc::tikzsave(
  here("figures/qld_wholesale.pdf"),
  plot_qld_wholesale, width = 10, height = 5
)

## Many Factor Estimators
kfbmisc::tikzsave(
  here("figures/retail_many_estimators.pdf"),
  plot_retail_many_estimators, width = 10, height = 5
)
kfbmisc::tikzsave(
  here("figures/wholesale_many_estimators.pdf"),
  plot_wholesale_many_estimators, width = 10, height = 5
)

## Covariates
kfbmisc::tikzsave(
  here("figures/retail_covs.pdf"),
  plot_retail_covs, width = 10, height = 5
)
kfbmisc::tikzsave(
  here("figures/wholesale_covs.pdf"),
  plot_wholesale_covs, width = 10, height = 5
)

## QLD Naive SEs
kfbmisc::tikzsave(
  here("figures/qld_retail_naive_se.pdf"),
  plot_qld_retail_naive_se, width = 10, height = 5
)
kfbmisc::tikzsave(
  here("figures/qld_wholesale_naive_se.pdf"),
  plot_qld_wholesale_naive_se, width = 10, height = 5
)

## Synthetic Control
kfbmisc::tikzsave(
  here("figures/synth_retail.pdf"),
  plot_synth_retail, width = 10, height = 5
)
kfbmisc::tikzsave(
  here("figures/synth_wholesale.pdf"),
  plot_synth_wholesale, width = 10, height = 5
)
