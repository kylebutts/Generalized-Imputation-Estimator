...
For all units $i$ and groups $g \in \mathcal{G}$, $y_{it} = y_{it}(\infty)$ for $t < g$. $\blacksquare$ % Or, less strictly, we require $\condexpec{y_{it}}{G_i = g} = \condexpec{y_{it}(\infty)}{G_i = g}$ for $t < g$.
\end{assumption}
